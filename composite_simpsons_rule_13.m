% Composite Simpson's 1/3 rule for approximations for definite integrals.
...
while(difference >= wanted_difference)
  ...
  f4 = f2 = 0;          % Sum counters
  for i = 2:step+1
    if(mod(i,2) == 0) f4 = f4 + f(matrix(i)); endif %4f
    if(mod(i,2) != 0) f2 = f2 + f(matrix(i)); endif %2f
  endfor

  current_result = (h/3)*(f(a) + (2*f2) + (4*f4) + f(b));
