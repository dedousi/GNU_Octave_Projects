% Conposite Simpson's 3/8 rule for approximations for definite integrals.
...
while(difference >= wanted_difference)
  ...
  f3 = f2 = 0;          % Sum counters
  for i = 2:step+1
    if(mod(i,3) != 0) f3 = f3 + f(matrix(i)); endif %3f
    if(mod(i,3) == 0) f2 = f2 + f(matrix(i)); endif %2f
  endfor

  current_result = ((3*h)/8)*(f(a) + (2*f2) + (3*f3) + f(b));
